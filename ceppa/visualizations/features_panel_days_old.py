...
def get_chow_to_HF_panel_data(experiment, feature_list, bin_type, level, err_type, 
        ...
        for c, feat in enumerate(feature_list):
            ...
            s_data, s_labels, m_data, m_labels, md_data, md_labels = \
                    E.generate_feature_vectors(
                        feat, bin_type='12bins', days=[day], 
                        group_avg_type=group_avg_type)   
...
def draw_features(E, avgs, errs, fig_title, ymaxs=None, ADD_SOURCE=False):
    ...
    for c in xrange(len(feats)):
        ...
        for group, ax in enumerate([ax0, ax1]):
            ...
            set_panel_layout(E, ax, feats, c, bin_type='12bins', ymaxs=ymaxs)
...
def plot_features_transition_days(experiment, bin_type='12bins', level='strain', 
    ...
    avgs, errs, labels = get_chow_to_HF_panel_data(E, feature_list, bin_type, level, err_type)
    ...
    print "%s..\nlevel: %s\nbin_type: %s\nerr_type: %s" %(
            plot_features_transition_days.__name__, level, bin_type, err_type)
